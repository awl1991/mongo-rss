...
async function fetchHeadlines() {
    ...
    // Create a rolling time window for the last 24 hours instead of strict UTC day boundaries
    const recentTimeWindow = new Date();
    recentTimeWindow.setHours(recentTimeWindow.getHours() - 24); // Last 24 hours
        ...
        const feedPromises = RSS_FEEDS.map(async (feed) => {
            try {
                ...
                for (const entry of feedData.items) {
                    ...
                    if (pubDate < recentTimeWindow) {
                        console.log(`  Skipped '${headline}' from ${feed.name} - Older than 24 hours (Pub time: ${pubTimeRaw})`);
                        continue;
                    }
