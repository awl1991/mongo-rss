...
export async function getHeadlines(limit = 200) {
...
export async function insertHeadline(headline) {
  const { db } = await connectToDatabase();
  try {
...
export async function getHeadlineByLink(link) {
  const { db } = await connectToDatabase();
  return db.collection('headlines').findOne({ link });
}

export async function deleteOldHeadlines(olderThan) {
  const { db } = await connectToDatabase();
...
export async function deleteAllHeadlines() {
  const { db } = await connectToDatabase();
  return db.collection('headlines').deleteMany({});
}

export async function getHeadlinesCount() {
  const { db } = await connectToDatabase();
